...
describe('Native Bottom Tabs Navigation', () => {
  ...
  beforeEach(() => {
    renderRouter({
      ...
      'not-specified': () => <View testID="not-specified" />,
    });
    expectOneRender();
  ...
  it('redirects to the initial tab when router.push targets a hidden or not-specified route', () => {
    ...
    act(() => router.push('/not-specified'));
    expect(lastHostSelectedKey()).toMatch(/^index-[-\w]+/);
    expect(screen).toHavePathname('/');
  });

  it('keeps hidden and not-specified routes registered in navigation state', () => {
    const tabState = store.navigationRef.getRootState().routes[0]!.state!;
    expect(tabState.routes.map((r) => r.name).sort()).toEqual([
      'hidden',
      'index',
      'not-specified',
      'second',
...
describe('Native Bottom Tabs route registration', () => {
  ...
  it('registers routes without visible triggers but does not show them as tabs', () => {
    ...
    expect(tabRouteNames().sort()).toEqual(['hidden', 'index', 'not-specified']);
    // Only the visible trigger becomes a tab item.
    expect(TabsScreen).toHaveBeenCalledTimes(1);
    expect(TabsScreen.mock.calls[0]![0].screenKey).toMatch(/^index-[-\w]+/);
  });

  it('keeps a route registered when its trigger is removed dynamically', () => {
    ...
    // Navigating to it redirects to the initial tab.
    act(() => router.push('/second'));
    expect(screen).toHavePathname('/');
  });
  ...
  it('respects initialRouteName when redirecting from a route without a visible tab', () => {
    renderRouter(
      {
        _layout: () => (
          <NativeTabs initialRouteName="second">
            <NativeTabs.Trigger name="index" />
            <NativeTabs.Trigger name="second" />
          </NativeTabs>
        ),
  ...
  it('respects an initialRouteName that targets a directory index route', () => {
    renderRouter(
      {
        _layout: () => (
          <NativeTabs initialRouteName="second/index">
            <NativeTabs.Trigger name="index" />
            <NativeTabs.Trigger name="second" />
          </NativeTabs>
        ),
  ...
  it('keeps the navigator mounted when every trigger is hidden', () => {
    renderRouter({
      _layout: () => (
        <NativeTabs>
