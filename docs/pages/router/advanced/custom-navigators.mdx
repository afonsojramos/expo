  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={{ flexDirection: 'row' }}>
        {state.routes.filter(route => descriptors[route.key].routeSource === 'layout').map(route => (
          <Pressable
            key={route.key}
...
Expo Router registers every route discovered from the filesystem, including routes without a matching `<Navigator.Screen>` declaration. Use each descriptor's `routeSource` to include only layout-declared routes in navigator UI while keeping inferred routes available for navigation.
...
| `descriptors` | A map keyed by `route.key`. Each descriptor exposes the screen's resolved `options`, a `render()` function that renders the screen, and a `routeSource`. The source is `layout` for a route declared with `<Navigator.Screen>`, `filesystem` for an inferred route, or `undefined` outside Expo Router. |
...
The `NavigatorContent` component shown above is a [standard navigator](https://github.com/react-navigation/standard-navigation). It implements a minimal, framework-agnostic contract defined by the [`standard-navigation`](https://www.npmjs.com/package/standard-navigation) package. Expo Router extends the standard descriptor with `routeSource`; use Expo Router's `NavigatorContentProps` or `StandardNavigatorDescriptor` types when reading it. The only other difference between the in-app and library integrations is who creates the navigator.
